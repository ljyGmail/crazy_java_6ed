...
public class F_ImageViewerTest {
    // 定义图片预览组件的大小
...
class ExtensionFileFilter extends FileFilter {
    ...
    // 继承FileFilter类必须实现的抽象方法，判断文件过滤器是否接收该文件
    public boolean accept(File f) {
        // 如果该文件是路径，则接收该文件
        if (f.isDirectory()) return true;
        // 将文件名转换为小写(全部转换为小写后比较，用于忽略文件名大小写)
        String name = f.getName().toLowerCase();
        // 遍历所有可接受的扩展名，如果扩展名相同，则该文件可被接收
...
class FileIconView extends FileView {
    ...
    public Icon getIcon(File f) {
        if (!f.isDirectory() && filter.accept(f)) {
            return new ImageIcon("../data/ch12_swing/ico/pict.png");
            ...
            for (var tmp : fList) {
                // 如果该路径是跟路径
                if (tmp.equals(f)) {
                    return new ImageIcon("../data/ch12_swing/ico/dsk.png");
                }
            }
            return new ImageIcon("../data/ch12_swing/ico/folder.png");
        } else {
            // 使用默认图标
            return null;
        }
